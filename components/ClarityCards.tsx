...
const ClarityCards: React.FC<ClarityCardsProps> = ({ onGoDashboard, onCreateNew }) => {
  ...
  const handleDownload = (card: typeof CARDS[0]) => {
    ...
    const content = `PACTO - KIT DE CLARIDAD
--------------------------------------------------
TÍTULO: ${card.title}
CATEGORÍA: ${card.category} | TIPO: ${card.type}
--------------------------------------------------
...
PASOS CLAVE:
${card.steps?.map((s, i) => `${i + 1}. ${s.title}: ${s.desc}`).join('\n')}

TIP DE INCLUSIÓN:
${card.tip}

--------------------------------------------------
Generado por PACTO by Teamworkz.
`;
    ...
    element.download = `PACTO_Plantilla_${card.title.replace(/\s+/g, '_')}.txt`;
