      ...
      <div className="flex-1 flex flex-col gap-8 min-w-0">
        ...
        <div className="bg-white rounded-xl p-8 shadow-sm border border-gray-border flex flex-col gap-6 relative overflow-hidden">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 relative z-10">
                <div className="flex flex-col gap-2">
                    <h1 className="text-text-n900 text-4xl font-black leading-tight tracking-tight">Nuestro Equipo</h1>
                    <p className="text-gray-600 text-lg font-medium">Encuentra a tus compañeros y descubre sus mejores formas de colaborar.</p>
                </div>
                {onNavigateToPrivacy && (
                    <button 
        ...
        <div className="flex flex-wrap gap-3 pb-4 px-1">
            <button 
                ...
                className={`flex h-12 items-center justify-center gap-x-2 rounded-xl px-6 font-bold transition-all ${
            ...
            >
                Todos <span className={`px-2 py-0.5 rounded-md text-xs font-bold ${
                    selectedDepts.length === 0 ? 'bg-white/20 text-white' : 'bg-gray-100 text-gray-500'
                }`}>
                    {members.length}
                </span>
            </button>
            ...
            {['Ingeniería', 'Diseño', 'Operaciones'].map(cat => (
                <button 
                    ...
                    className={`flex h-12 items-center justify-center gap-x-2 rounded-xl px-6 font-bold transition-all ${
                        selectedDepts.includes(cat) && selectedDepts.length === 1
                        ? 'bg-primary text-white shadow-lg shadow-primary/25' 
                        : 'bg-white text-gray-700 border border-gray-200 shadow-sm hover:border-primary/30 hover:text-primary hover:shadow-md'
                    }`}
                >
