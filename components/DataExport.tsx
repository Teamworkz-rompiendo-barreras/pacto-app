...
const DataExport: React.FC<DataExportProps> = ({ onBack }) => {
  ...
  return (
    <div className="w-full h-full animate-fade-in font-display bg-bg-s1 min-h-screen flex flex-col">
      {/* Header */}
      <header className="w-full flex items-center justify-between border-b border-primary/10 bg-white px-6 py-4 lg:px-10 z-10 sticky top-0">
        <div className="flex items-center gap-3 text-text-n900 cursor-pointer" onClick={onBack}>
            <div className="flex items-center justify-center size-10 rounded-lg bg-primary text-white">
                <span className="material-symbols-outlined text-2xl">diversity_3</span>
            </div>
            <h2 className="text-xl font-black tracking-tight uppercase">PACTO</h2>
        </div>
        <button 
            onClick={onBack}
            className="group flex items-center gap-2 rounded-xl border border-gray-200 bg-white px-5 py-2.5 text-sm font-bold text-gray-600 transition-colors hover:bg-gray-50 hover:text-primary hover:border-primary/30"
        >
