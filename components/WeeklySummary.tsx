...
const WeeklySummary: React.FC<WeeklySummaryProps> = ({ onNavigateToCards }) => {
  ...
  useEffect(() => {
    const loadData = async () => {
      const summary = await reportService.getWeeklySummary('current-user');
      setData(summary);
    };
    loadData();
  }, []);

  if (!data) return <div className="p-10 text-center animate-pulse">Cargando reporte semanal...</div>;
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-8 mt-auto z-10">
              <div className="flex flex-col gap-1 p-4 rounded-xl hover:bg-bg-s1/50 transition-colors cursor-default border border-transparent hover:border-gray-100">
                ...
                <div className="flex items-baseline gap-2">
                  <span className="text-6xl font-black text-primary tracking-tighter">{data.ritualsCompleted}</span>
              ...
              <div className="flex flex-col gap-1 p-4 rounded-xl hover:bg-bg-s1/50 transition-colors cursor-default border border-transparent hover:border-gray-100">
                ...
                <div className="flex items-baseline gap-2">
                  <span className="text-6xl font-black text-primary tracking-tighter">{data.agreementsCreated}</span>
          ...
          <div className="lg:col-span-5 bg-white rounded-2xl shadow-sm border border-gray-100 p-8 flex flex-col">
            ...
            <div className="flex flex-col gap-4">
              {data.topAchievements.map((achievement, idx) => (
                <label key={idx} className="flex items-start gap-4 p-4 rounded-xl border-2 border-transparent hover:border-bg-s1 hover:bg-bg-s1/30 transition-all cursor-pointer group">
                  ...
                  <div className="flex flex-col">
                    <span className="text-text-n900 font-bold text-lg leading-tight group-hover:text-primary transition-colors">{achievement}</span>
                    <span className="text-gray-500 text-sm mt-1 font-medium">Contribución confirmada</span>
                  </div>
                </label>
              ))}
            </div>
          ...
          <div className="lg:col-span-7 bg-white rounded-2xl shadow-sm border border-gray-100 p-8 flex flex-col">
            <div className="flex flex-wrap items-center justify-between gap-4 mb-8">
              ...
              <div className="flex items-center gap-2">
                <span className="text-3xl font-black text-primary">{data.focusHours.reduce((a, b) => a + b, 0).toFixed(1)}</span>
