...
const Login: React.FC<LoginProps> = ({ onLogin, onCancel }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (mode === 'LOGIN') {
       // Mock login
       onLogin('Alex Rivera', settings);
    } else if (mode === 'SIGNUP') {
       if (name) onLogin(name, settings);
    } else {
  ...
  return (
    <div className="w-full max-w-md mx-auto animate-fade-in">
      <section className="bg-white p-8 rounded-2xl shadow-xl border border-gray-100">
        ...
        {notification && (
            <div className="mb-6 p-4 bg-green-50 text-green-700 rounded-xl text-sm font-bold flex items-center gap-2 border border-green-100">
